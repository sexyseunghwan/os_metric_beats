...
fn get_env_var_with_logging(var_name: &str) -> String {
    match env::var(var_name) {
        ...
        Err(env::VarError::NotPresent) => {
            ...
            panic!("[ERROR][get_env_var_with_logging] Environment variable '{}' must be set", var_name);
        }
...
pub static ELASTIC_SERVER_INFO: once_lazy<String> = once_lazy::new(|| {
    get_env_var_with_logging("ELASTIC_SERVER_INFO")
});

#[doc = "Function to globally initialize the 'SYSTEM_INFO' variable"]
pub static SYSTEM_INFO: once_lazy<String> = once_lazy::new(|| {
    get_env_var_with_logging("SYSTEM_INFO")
});

#[doc = "Function to globally initialize the 'LINUX_CONFIG_INFO' variable"]
pub static LINUX_CONFIG_INFO: once_lazy<String> = once_lazy::new(|| {
    get_env_var_with_logging("LINUX_CONFIG_INFO")
});